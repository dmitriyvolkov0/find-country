...
/**
 * Модальное окно подтверждения использования +30 секунд
 */
export function TimeBonusModal() {
  const { showTimeBonusConfirmModal, setShowTimeBonusConfirmModal, addTimeBonus } = useGameStore();
  const { stars } = useStars();

  const TIME_BONUS_COST = 5;

  // Если модальное окно не показано, не рендерим
  if (!showTimeBonusConfirmModal) {
    return null;
  }

  /**
   * Обработка подтверждения использования +30 секунд
   */
  const handleConfirm = async () => {
    setShowTimeBonusConfirmModal(false);
    await addTimeBonus();
  };
      ...
      >
        <div className="bg-white rounded-t-2xl shadow-2xl p-4 sm:p-5 mx-2 sm:mx-auto sm:max-w-md">
          ...
          <div className="flex items-center justify-center gap-2 mb-4">
            ...
            <span className="text-gray-600 font-medium">Стоимость: {TIME_BONUS_COST} звёзд</span>
          </div>
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              disabled={stars < TIME_BONUS_COST}
              className="flex-1 h-12 rounded-xl bg-green-100 hover:bg-green-200 active:bg-green-300 disabled:bg-gray-300 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-all duration-200 transform hover:scale-[1.02] active:scale-[0.98] focus:outline-none focus:ring-2 focus:ring-green-400 focus:ring-offset-2"
            ...
            >
              ...
              <span className="text-green-700 font-medium">Использовать</span>
            </button>
          </div>
        </div>
      </div>
