...
export function GameMenu() {
  const { phase, setShowHintConfirmModal, setShowTimeBonusConfirmModal } = useGameStore();
  ...
  return (
    <>
      ...
      <button
        onClick={toggleMenu}
        className="absolute bottom-20 left-4 sm:bottom-24 sm:left-6 z-10 p-3 sm:p-4 rounded-full shadow-lg transform transition-all duration-200 hover:scale-105 active:scale-95 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2 focus:ring-offset-blue-900 bg-gradient-to-r from-blue-500 to-cyan-600 hover:from-blue-400 hover:to-cyan-500"
        aria-label="Меню"
        title="Меню"
      >
      ...
      {isMenuOpen && (
        <>
          ...
          {/* Панель меню */}
          <div className="absolute bottom-40 left-4 sm:left-6 z-30 bg-white rounded-2xl shadow-2xl overflow-hidden animate-slide-up">
            {/* Кнопка подсказки */}
            <button
              onClick={handleHint}
