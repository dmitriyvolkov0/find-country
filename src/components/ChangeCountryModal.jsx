 ...
 */
export function ChangeCountryModal() {
  const { showChangeCountryConfirmModal, setShowChangeCountryConfirmModal, changeCountry } = useGameStore();
  const { stars } = useStars();

  const CHANGE_COUNTRY_COST = 2;

  // Если модальное окно не показано, не рендерим
  if (!showChangeCountryConfirmModal) {
    return null;
  }

  /**
   * Обработка подтверждения смены страны
   */
  const handleConfirm = async () => {
    setShowChangeCountryConfirmModal(false);
    await changeCountry();
  };
      ...
      >
        <div className="bg-white rounded-t-2xl shadow-2xl p-4 sm:p-5 mx-2 sm:mx-auto sm:max-w-md">
          ...
          <div className="flex items-center justify-center gap-2 mb-4">
            ...
            <span className="text-gray-600 font-medium">Стоимость: {CHANGE_COUNTRY_COST} звезды</span>
          </div>
          ...
          <div className="flex gap-3">
            ...
            {/* Кнопка подтверждения (фиолетовая) */}
            <button
              onClick={handleConfirm}
              disabled={stars < CHANGE_COUNTRY_COST}
              className="flex-1 h-12 rounded-xl bg-purple-100 hover:bg-purple-200 active:bg-purple-300 disabled:bg-gray-300 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-all duration-200 transform hover:scale-[1.02] active:scale-[0.98] focus:outline-none focus:ring-2 focus:ring-purple-400 focus:ring-offset-2"
            ...
            >
              ...
              <span className="text-purple-700 font-medium">Сменить</span>
            </button>
          </div>
        </div>
      </div>
